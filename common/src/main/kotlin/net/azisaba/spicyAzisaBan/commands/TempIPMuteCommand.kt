...
object TempIPMuteCommand: Command() {
    ...
    override val permission = PunishmentType.TEMP_IP_MUTE.perm
    ...
    override fun execute(actor: Actor, args: Array<String>) {
        ...
        async<Unit> { context ->
            doTempIPMute(actor, arguments, actor.getServerOrGroupName(arguments))
            context.resolve()
        }.catch {
    ...
    internal fun doTempIPMute(actor: Actor, arguments: ArgumentParsedResult, serverName: String? = null) {
        ...
        val p = Punishment
            .createByIPAddress(ip, reason.text, actor.uniqueId, PunishmentType.TEMP_IP_MUTE, Expiration.ExpireAt.of(System.currentTimeMillis() + time), server.name)
            .insert(actor)
            .catch {
