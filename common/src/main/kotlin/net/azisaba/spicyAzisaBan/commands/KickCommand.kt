...
object KickCommand: Command() {
    ...
    override fun execute(actor: Actor, args: Array<String>) {
        ...
        if (args.isEmpty()) return actor.send(SABMessages.Commands.Kick.usage.replaceVariables().translate())
        val arguments = genericArgumentParser.parse(args.joinToString(" "))
        async<Unit> { context ->
            doKick(actor, arguments, actor.getServerOrGroupName(arguments))
            context.resolve()
        }.catch {
    ...
    internal fun doKick(actor: Actor, arguments: ArgumentParsedResult, serverName: String? = null) {
    ...
    fun doKick(actor: Actor, player: PlayerContext, server: ServerContext, reason: ReasonContext) {
        val p = Punishment
            .createByPlayer(player.profile, reason.text, actor.uniqueId, PunishmentType.KICK, Expiration.NeverExpire, server.name)
            .insert(actor)
            .catch {
        ...
        actor.send(SABMessages.Commands.Kick.done.replaceVariables(p.getVariables().complete()).translate())
    }
