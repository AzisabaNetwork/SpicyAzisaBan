...
object GlobalWarningCommand: Command("${SABConfig.prefix}gwarning", null, "gwarn"), TabExecutor {
    ...
    override fun execute(sender: CommandSender, args: Array<String>) {
        ...
        if (args.isEmpty()) return sender.send(SABMessages.Commands.Warning.globalUsage.replaceVariables().translate())
        val arguments = ArgumentParser(args.joinToString(" "))
        Promise.create<Unit> { context ->
            WarningCommand.doWarning(sender, arguments)
            context.resolve()
        }.catch {
    ...
    override fun onTabComplete(sender: CommandSender, args: Array<String>): Iterable<String> {
        ...
        if (s.startsWith("reason=")) return ReasonContext.tabComplete(PunishmentType.WARNING, args, "global")
        return emptyList()
    }
}
